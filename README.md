...
Key Features
PDF Analysis: Seamlessly upload and process complex PDF documents.
Natural Language Querying: Ask questions in plain English and get precise answers.
RAG Architecture: Reduces hallucinations by grounding responses in the provided document context.
Privacy-Focused: Uses Ollama for local embeddings and LLM execution—no data leaves your machine.
Optimized Performance: Features vector store persistence and Streamlit caching for faster response times.
User-Friendly UI: Simple, intuitive interface built with Streamlit.
